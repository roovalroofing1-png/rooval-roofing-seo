...
- **GTM audit done (2026-06-01):** GTM container = **GTM-K9Q9S982** (loaded by GTM Kit; managed at tagmanager.google.com). The only GA tag firing uses Measurement ID **`G-TESTM`** — a **placeholder**, so Google Analytics is almost certainly NOT recording (and ~135KB of gtag.js is loading for nothing). The `G-TESTM` tag lives **inside the GTM container**, not in WordPress → fix in the GTM dashboard: replace `G-TESTM` with the real GA4 ID, or delete the tag. The **Facebook Pixel `470867249167206` is real/active** — keep it.

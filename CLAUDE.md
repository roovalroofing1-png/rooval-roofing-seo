...
### 2026-07-13 — WS2 rank-push deploy (partial; openings+titles+metas live)
Deployed & curl-verified: 4 Rank Math title changes (Orem/Murray/Draper/PG → noun-matched "…Roofing … & Roof Repair | 5.0★ Rated" pattern), 9 meta descriptions (Alpine broken-fragment fixed), and 6 answer-first body openings (Sandy/Provo/Orem/Lehi[prepend]/Murray/Draper — prepended to the under-hero text-editor widget, original copy kept, single H1 preserved). Pipeline: Rank Math `updateMetaBulk` (one page/call, all alias keys) + Elementor `save_builder` AJAX (fetch `meta._elementor_data` via `wp/v2/pages/{id}?context=edit`, prepend `<p>` to the largest text-editor widget, POST admin-ajax with `elementorCommonConfig.ajax.nonce`). ⚠️ **STALE LIVE FAQ PRICES FOUND:** WPCode snippet **2621** (city FAQ) on the LIVE site still has OLD prices for all 5 priced cities; the repo copy has NEWER prices that were never deployed live. Doc's price "targets" were the repo (undeployed) values → briefly created a body-vs-FAQ conflict → **reverted all 5 body prices to match live FAQ** (net pricing unchanged, consistent). **Owner must decide which prices are current** before syncing (2621 = the white-screen-risk PHP snippet — back up first). ⏭️ NOT done: internal-link map (~17 net-new links + service-page anchors), homepage global meta description (Rank Math → Titles & Meta → Homepage, since it's the static front page), homepage body links. Full record: `docs/ws2-deploy-log-2026-07-13.md` + strategy `docs/ws2-deployment-doc-2026-07-13.md`. ⚙️ Homepage per-page Rank Math description does NOT render (front-page uses the global Homepage setting).
...
- **Homepage de-clutter (1723) — LIVE & verified:** (1) removed the stock-video section; (2) shrank the hero from `min_height` 100vh → 60vh desktop / 62vh tablet / 68vh mobile + trimmed bottom padding (150→70) so the photo no longer fills the whole screen; hero CTA → /roof-quote/ preserved. Elementor revisions saved for rollback.
- **Contact form → conversational multi-step funnel (Elementor Pro Form in shared template "get a quote", post 2229; form widget 14ffcaa8):** owner wanted the long 8-field form turned into a click-through funnel that still collects all the info. Rebuilt as a **3-step funnel** via `form_fields` repeater (`step_type` was already `number_text`): **Step 1 "Your Project"** = What do you need? (select: Roof Replacement / Roof Repair / Roof Tune-Up / Roof Inspection / Gutters / Not sure) + Property type (Residential/Commercial); **Step 2 "Property Location"** = Street address + Zip; **Step 3 "Your Info"** = Name + Phone + Email + optional message. Verified live (progress bar + Next/Previous work). ⚠️ **Template 2229 is SHARED — used on BOTH the homepage (1723) and Contact page (42)**, so both now show the funnel. Delivery unchanged: `email_content=[all-fields]` → emails info@rooval-roofing.com + roovalroofing1@gmail.com + save-to-database (Elementor Submissions). CRM-swappable later. (Briefly had it cut to 4 fields before the owner clarified they wanted all fields kept in a funnel.)
- **Funnel strategy decision (2026-06-09):** investigated AYS's "Free Inspection" funnel — it is NOT a WordPress build. It's a **paid stack embedded via iframe: Alivo.ai** (AI roofing funnel/CRM — the auto-advancing card quiz + "We've Got You Covered" screen), **Roofle** (their instant-quote widget, = our /roof-quote/ tool), and **GoHighLevel / LeadConnector** (booking + CRM). That polish = a paid CRM/funnel platform. Owner's decision: **HOLD further funnel work for the CRM** — when a CRM is chosen, use a roofing funnel platform (Alivo / Roofle / GoHighLevel) for the full AYS-style funnel + online booking. The current 3-step Elementor funnel stays as the interim; do NOT invest more in the WP funnel. ✅ This is the home for the deferred "simplify form" / "conversational funnel" work.
- **Free Roof Tune-Up / Repair page — BUILT & LIVE** at `/roof-tune-up/` (**page id 2626**; content mirrored in `pages/roof-tune-up-2626.html`). Standard WP page, self-contained inline CSS (`.rrtu`), on-brand red/gold. Sections: hero → what-is-a-tune-up → what's-included checklist → leak/storm repair callout → why-it-pays-off → CTA band. CTAs funnel to `/roof-quote/` + tel:(385) 424-8810 (interim; repoint to the **CRM** when chosen — owner's plan). Uses "workmanship warranty," no "15-year". **Linked in nav** under "Our Services" (menu item 2627, parent 57 — owner chose the dropdown placement). **Rank Math SEO set** (title "Roof Tune-Up & Repair in Lehi, UT | Rooval Roofing" + description) via run-once snippet `snippets/set-tune-up-seo-meta-2629.php` (id 2629, deactivated after run; meta persists in DB). Verified live.
